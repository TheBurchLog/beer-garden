...
stomp:
  ...
  username: beer_garden"""
    ...
    def test_garden_unresponsive_trigger(self, bg_garden):
        bg_garden.systems = []
        for connection in bg_garden.receiving_connections:
            connection.status_info.heartbeat = datetime.now(timezone.utc) - timedelta(minutes=60)
    ...
    def test_garden_unresponsive_trigger_in_window(self, bg_garden):
        bg_garden.systems = []
        for connection in bg_garden.receiving_connections:
            connection.status_info.heartbeat = datetime.now(timezone.utc) - timedelta(minutes=10)
